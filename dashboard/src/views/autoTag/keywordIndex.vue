<template>
  <a-card>
    <div class="tag-box">
      <div class="page_top">
        <a-button type="primary" @click="$router.push('/autoTag/keywordCreate')">添加规则</a-button>
      ...
      <div class="rule-list">
        <a-table :columns="table.col" :data-source="table.data">
          <div slot="fuzzy_match_keyword" slot-scope="text" v-if="text!=''">
            <a-tag v-for="(item,index) in text" :key="index">{{ item }}</a-tag>
          </div>
          <div slot="exact_match_keyword" slot-scope="text" v-if="text!=''">
            <a-tag v-for="(item,index) in text" :key="index">{{ item }}</a-tag>
          </div>
          ...
          <div slot="operate" slot-scope="text,record">
            <span>
              <a @click="$router.push('/autoTag/keywordShow?idRow='+record.id)">详情</a>
    ...
    return {
      ...
      table: {
        col: [
          ...
          {
            key: 'fuzzy_match_keyword',
            dataIndex: 'fuzzy_match_keyword',
            title: '模糊匹配',
            scopedSlots: { customRender: 'fuzzy_match_keyword' }
          },
          {
            key: 'exact_match_keyword',
            dataIndex: 'exact_match_keyword',
            title: '精准匹配',
            scopedSlots: { customRender: 'exact_match_keyword' }
          },
          {
            key: 'tags',
            dataIndex: 'tags',
