      ...
      <div class="preview-form-box flex" v-show="form.pushContact">
        <div class="form mr80">
          ...
          <div class="item">
            ...
            <m-enter-text ref="enterText" v-model="form.msgText"/>
          </div>
          <div class="item">
            ...
            <div class="content">
              ...
              <div class="operating">
                <div class="image" v-if="msgType.value === 'image'">
                  <m-upload ref="image" v-model="form.msgComplex.image"></m-upload>
                </div>
...
export default {
  ...
  methods: {
    setOrgData (data) {
      ...
      this.form.pushContact = push.push_contact === 'true'
      this.form.pushEmployee = push.push_employee === 'true'
      this.$refs.enterText.addUserName(push.msg_text)
      this.msgType.value = push.msg_complex_type
      if (push.msg_complex) {
        const msgComplex = push.msg_complex
        if (push.msg_complex_type === 'image') {
          this.$refs.image.setUrl(msgComplex.image)
        }
        if (push.msg_complex_type === 'link') {
          this.$refs.image.setUrl(msgComplex.image)
        }
      }
    },
