<template>
  <div class="interactive_radar_pdf">
    <a-card class="mb16" title="基础信息">
      <div class="form">
        <div class="item must">
          <span>雷达标题：</span>
          <div class="input">
            <a-input placeholder="名称不会展示给用户，用于企业记录渠道名称或使用场景" disabled v-model="formAskData.title"></a-input>
          </div>
        </div>
        <div class="item must">
          <span>雷达PDF：</span>
    ...
    <a-card title="链接追踪设置">
      ...
      <div class="btn mt32">
        <a-button type="primary" size="large" @click="pdfSetup">修改雷达PDF</a-button>
      </div>
    </a-card>
  </div>
</template>
...
import { infoApi, updateApi } from '@/api/radar'
  ...
  methods: {
    ...
    getDataPdf (id) {
      infoApi({ id }).then((res) => {
        ...
        this.formAskData.pdf = res.data.pdf
        this.formAskData.pdf_name = res.data.pdfName
        //  包装后台数据
