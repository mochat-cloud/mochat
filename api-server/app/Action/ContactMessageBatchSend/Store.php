...
use App\Logic\ContactMessageBatchSend\StoreLogic;
...
class Store extends AbstractAction
{
    ...
    /**
     * @RequestMapping(path="/contactMessageBatchSend/store", methods="POST")
     */
    ...
    protected function validContent(array $content): void
    {
        ...
        foreach ($content as $item) {
            if (empty($item['msgType']) && (empty($item['text']) || empty($item['text']['content']))) {
                throw new CommonException(ErrorCode::INVALID_PARAMS, '消息类型不能为空');
            }
            if (!empty($item['text']) && !empty($item['text']['content']) && strlen($item['text']['content']) > 4000) {
            ...
            switch ($item['msgType']) {
                ...
                case 'link':
                    ...
                    if (!empty($item['desc']) && strlen($item['desc'])) {
