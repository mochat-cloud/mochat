<?php

declare(strict_types=1);
/**
 * This file is part of MoChat.
 * @link     https://mo.chat
 * @document https://mochat.wiki
 * @contact  group@mo.chat
 * @license  https://github.com/mochat-cloud/mochat/blob/master/LICENSE
 */
namespace MoChat\App\WorkContact\EventHandler\Employee;

use MoChat\App\WorkContact\Constants\Employee\Status;
use MoChat\App\WorkContact\Logic\DestroyCallBackLogic;
use MoChat\Framework\Annotation\WeChatEventHandler;
use MoChat\Framework\WeWork\EventHandler\AbstractEventHandler;

/**
 * 删除跟进成员 - 事件回调.
 * @WeChatEventHandler(eventPath="event/change_external_contact/del_follow_user")
 * Class DestroyHandler
 */
class DestroyHandler extends AbstractEventHandler
{
    /**
     * @return null|mixed|void
     */
    public function process()
    {
        ## 队列
        make(DestroyCallBackLogic::class)->handle($this->message, Status::PASSIVE_REMOVE);
    }
}
