...
class UpdateContactCallBackApply
{
    ...
    public function handle(array $wxResponse): void
    {
        ...
        if (isset($wxResponse['UserID'], $wxResponse['ExternalUserID'])) {
            ...
            $corpId     = 0;
            //查询客户id
            $contactInfo = make(WorkContactContract::class)->getWorkContactByWxExternalUserId($wxResponse['ExternalUserID'], ['id']);
            if (! empty($contactInfo)) {
                $contactId = $contactInfo['id'];
            }

            //查询员工id
            $employeeInfo = make(WorkEmployeeContract::class)->getWorkEmployeeByWxUserId($wxResponse['UserID'], ['id']);
            if (! empty($employeeInfo)) {
                $employeeId = $employeeInfo['id'];
            }

            //查询企业id
            $corpInfo = make(CorpContract::class)->getCorpsByWxCorpId($wxResponse['ToUserName'], ['id']);
            if (! empty($corpInfo)) {
                $corpId = $corpInfo['id'];
            }
