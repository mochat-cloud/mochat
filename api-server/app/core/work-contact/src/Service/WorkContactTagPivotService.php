...
class WorkContactTagPivotService extends AbstractService implements WorkContactTagPivotContract
{
    /**
     ...
     * 查询客户标签.
     */
    public function getWorkContactTagPivotsNameByContactId(int $contactId): array
    {
        $res = $this->model::from($this->model::query()->getModel()->getTable() . ' as work_contact_tag_pivot')
            ->join(WorkContactTag::query()->getModel()->getTable() . ' as ct', 'work_contact_tag_pivot.contact_tag_id', 'ct.id')
            ->where('work_contact_tag_pivot.contact_id', $contactId)
            ->distinct()
            ->get([
                'ct.name',
            ]);
