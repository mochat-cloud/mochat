...
class WorkContactRoomService extends AbstractService implements WorkContactRoomContract
{
    /**
     * @var WorkContactRoom
     */
    ...
    /**
     * @param int $roomId 群聊ID
     * @return array 响应数组
     */
    public function getWorkContactRoomsByRoomIdContact(int $roomId, array $columns = ['*']): array
    {
        $res = $this->model::query()
     ...
     * @param int $roomId 群聊ID
     */
    public function countWorkContactRoomsByRoomIdContact(int $roomId): int
    {
        return $this->model::query()
            ->where('room_id', $roomId)
            ->where('contact_id', '>', 0)
    ...
    /**
     * 计算企业总群人数.
     * @param int $roomId 群id
     * @param int $employee 员工
     * @param int $status 状态
     * @return int 返回值
     */
     ...
     * 获取群内员工.
     */
    public function getWorkContactRoomsByRoomIdEmployee(int $roomId, array $columns = ['*']): array
    {
        $res = $this->model::query()
            ->where('room_id', $roomId)
            ->where('employee_id', '>', 0)
     ...
     */
    public function getWorkContactRoomsByRoomIdUnion(int $roomId, string $unionId, array $columns = ['*']): array
    {
        $res = $this->model::query()
            ->where('room_id', $roomId)
