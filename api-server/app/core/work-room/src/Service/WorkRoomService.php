...
class WorkRoomService extends AbstractService implements WorkRoomContract
{
    /**
     * @var WorkRoom
     */
     ...
     */
    public function getWorkRoomsByCorpIdOwnerIds(int $corpId, array $params, array $columns = ['*']): array
    {
        $res = $this->model::query()
            ->where('corp_id', $corpId);

        if (isset($params['employees'])) {
            $res = $res->whereIn('owner_id', $params['employees']);
        }
        if (isset($params['name'])) {
            $res = $res->where('name', 'like', "%{$params['name']}%");
        }
