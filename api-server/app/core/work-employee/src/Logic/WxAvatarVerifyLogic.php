...
class WxAvatarVerifyLogic
{
    ...
    {
        $employees = $this->employeeService->getWorkEmployeeList(
            ['corp_id' => $corpId],
        ...
        foreach ($employees['data'] as $k => $employee) {
            ...
            $dbData[] = [
                'id'           => $employee['id'],
                'avatar'       => $this->asyncUpload($remoteEmployee['avatar'], 'mochat/employee/avatar/' . $fileName),
                'thumb_avatar' => $this->asyncUpload($remoteEmployee['thumb_avatar'], 'mochat/employee/thumb_avatar/' . $fileName),
            ];
        }
        empty($dbData) || $this->employeeService->updateWorkEmployeesCaseIds($dbData);
        return false;
    }
    ...
    {
        $contactList = $this->contactService->getWorkContactList(
            ['corp_id' => $corpId],
        ...
        foreach ($contactList['data'] as $k => $contact) {
            ...
            $dbData[] = [
                'id'     => $contact['id'],
                'avatar' => $this->asyncUpload($remoteContact['external_contact']['avatar'], 'mochat/contact/avatar/' . $fileName),
            ];
        }
        empty($dbData) || $this->contactService->updateWorkContactsCaseIds($dbData);
        return false;
    }
     ...
     * @return string 文件路径
     */
    protected function asyncUpload(string $url, string $path = ''): string
    {
        if (empty($url)) {
            return '';
        }
        $this->fileSystem->fileExists($path) && $this->fileSystem->delete($path);
