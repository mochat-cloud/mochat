...
class SynContactLogic
{
                ...
                if (! empty($item['tags'])) {
                    foreach ($item['tags'] as $v) {
                        ...
                        if (! empty($groupInfo)) {
                            ...
                            //添加客户、标签关联表
                            $contactTagPivotRes = $this->workContactTagPivot->createWorkContactTagPivot($addContactTag);
                            if (! is_int($contactTagPivotRes)) {
            ...
            foreach ($val['follow_user'] as $item) {
                ...
                if (! empty($item['tags'])) {
                    foreach ($item['tags'] as $v) {
                        $wxTagId = 0;
                        //根据分组名查询分组id
                        $groupInfo = $this->workContactTagGroup->getWorkContactTagGroupByCorpIdName($this->corpId, $v['group_name'], ['id']);
                        if (! empty($groupInfo)) {
                            $groupId = $groupInfo['id'];
                            if ($groupId != 0) {
                                //根据分组id和标签名称查询标签id
    ...
    {
        //查询当前企业有没有同步客户的时间
        $workUpdateTime = $this->workUpdateTime->getWorkUpdateTimeByCorpIdType([$this->corpId], (int) Type::CONTACT);
        //如果查到 就更新
