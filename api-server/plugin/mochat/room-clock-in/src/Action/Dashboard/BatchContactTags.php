...
class BatchContactTags extends AbstractAction
{
     ...
     * })
     * @RequestMapping(path="/dashboard/roomClockIn/batchContactTags", methods="put")
     */
    public function handle(): array
    {
        ...
        try {
            ...
            foreach ($params['ids'] as $item) {
                ## 客户id
                $contactId = $this->clockInContactService->getClockInContactById((int) $item, ['contact_id']);
                if ($contactId['contactId'] > 0) {
                    ...
                    $this->autoTag($data);
                    $this->clockInContactService->updateClockInContact([$item], ['contact_clock_tags' => json_encode($params['tags'], JSON_THROW_ON_ERROR)]);
                }
            }
            Db::commit();
        } catch (\Throwable $e) {
