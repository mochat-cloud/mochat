...
use MoChat\Plugin\RoomSop\Model\RoomSop;

class RoomSopService extends AbstractService implements RoomSopContract
{
    /**
     * @var RoomSop
     */
     ...
     * @return bool 执行结果
     */
    public function clearTableAndCreateRoomSops(int $corpId, array $data): bool
    {
        //清除本企业保存的所有数据
        ...
        //新增数据
        return $this->createRoomSops($data);
    }
     ...
     * @return array 数组
     */
    public function getRoomSopByCorpIdState(array $corpIds, int $state = 1, array $columns = ['*']): array
    {
        $res = $this->model::query()
            ->whereIn('corp_id', $corpIds)
            ->where('state', $state)
