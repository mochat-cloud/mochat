...
class Destroy extends AbstractAction
{
     ...
     * })
     * @RequestMapping(path="/dashboard/roomRemind/destroy", methods="DELETE")
     */
    public function handle(): array
    {
        ...
        Db::beginTransaction();
        try {
            $this->roomRemindService->deleteRoomRemind($id);
            Db::commit();
        } catch (\Throwable $e) {
            Db::rollBack();
            $loggers = $this->container->get(StdoutLoggerInterface::class);
            $loggers->error(sprintf('%s [%s] %s', '客户群提醒删除失败', date('Y-m-d H:i:s'), $e->getMessage()));
            $loggers->error($e->getTraceAsString());
            throw new CommonException(ErrorCode::SERVER_ERROR, '客户群提醒删除失败' . $e->getMessage()); //$e->getMessage()
        }
