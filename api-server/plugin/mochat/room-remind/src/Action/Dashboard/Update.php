...
class Update extends AbstractAction
{
     ...
     * })
     * @RequestMapping(path="/dashboard/roomRemind/update", methods="put")
    ...
    private function updateRoomRemind(int $id, array $params): array
    {
        ...
        try {
            ## 创建活动
            $this->roomRemindService->updateRoomRemindById($id, $params);

            Db::commit();
        } catch (\Throwable $e) {
            Db::rollBack();
            $this->logger->error(sprintf('%s [%s] %s', '客户群提醒修改失败', date('Y-m-d H:i:s'), $e->getMessage()));
