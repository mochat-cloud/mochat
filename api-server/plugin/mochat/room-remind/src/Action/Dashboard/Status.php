...
class Status extends AbstractAction
{
     ...
     * })
     * @RequestMapping(path="/dashboard/roomRemind/status", methods="get")
    ...
    public function handle(): array
    {
        ...
        try {
            ## 创建活动
            $this->roomRemindService->updateRoomRemindById((int) $params['id'], ['status' => (int) $params['status']]);

            Db::commit();
        } catch (\Throwable $e) {
            Db::rollBack();
            $this->logger->error(sprintf('%s [%s] %s', '客户群状态修改失败', date('Y-m-d H:i:s'), $e->getMessage()));
