 ...
 */
namespace MoChat\Plugin\RoomAutoPull\QueueService;

use Hyperf\AsyncQueue\Annotation\AsyncQueueMessage;
use Hyperf\Contract\StdoutLoggerInterface;
use Hyperf\Di\Annotation\Inject;
use MoChat\App\Corp\Contract\CorpContract;
use MoChat\Plugin\ChannelCode\QueueService\Traits\ContactWayQrCodeTrait;
...
class QrCodeUpdateApply
{
    ...
    {
        ## 获取自动拉群详情
        $workRoomAutoPull = make(WorkRoomAutoPullContract::class)->getWorkRoomAutoPullById($workRoomAutoPullId, ['wx_config_id']);
    ...
    private function handleQrCode(array $wxQrCode, int $workRoomAutoPullId)
    {
        try {
            ## 更新到数据表
            make(WorkRoomAutoPullContract::class)->updateWorkRoomAutoPullById($workRoomAutoPullId, ['qrcode_url' => $wxQrCode['qrCodePath'], 'wx_config_id' => $wxQrCode['wxConfigId']]);
        } catch (\Exception $e) {
