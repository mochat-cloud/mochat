 ...
 */
namespace MoChat\Plugin\SensitiveWord\QueueService;
...
use MoChat\Plugin\SensitiveWord\Constants\ReceiverType;
use MoChat\Plugin\SensitiveWord\Constants\Source;
...
class MonitorMessage
{
    ...
    public function handle(array $wxMsgArr): void
    {
        ...
        foreach ($wxMsgArr as $wxMsg) {
            ...
            $messageList = make(WorkMessageContract::class, [(int) $wxMsg['corpId']])->getWorkMessagesByMsgId($wxMsg['msgIds']);
    ...
    private function getChatContent(int $corpId, array $message)
    {
        if (! empty($message['wx_room_id'])) {
            ## 查询此条信息前十条
            $beforeList = make(WorkMessageContract::class, [(int) $corpId])->getWorkMessagesRangeByCorpIdWxRoomId($corpId, $message['wx_room_id'], $message['id'], '<', 10, 'id desc', ['id', 'from', 'msg_type', 'content', 'msg_time']);
            ## 查询此条信息后十条
            $afterList = make(WorkMessageContract::class, [(int) $corpId])->getWorkMessagesRangeByCorpIdWxRoomId($corpId, $message['wx_room_id'], $message['id'], '>', 10, 'id asc', ['id', 'from', 'msg_type', 'content', 'msg_time']);
        } else {
            ## 查询此条信息前十条
            $beforeList = make(WorkMessageContract::class, [(int) $corpId])->getWorkMessagesRangeByCorpId($corpId, $message['from'], json_decode($message['tolist'], true)[0], $message['id'], '<', 10, 'id desc', ['id', 'from', 'msg_type', 'content', 'msg_time']);
            ## 查询此条信息后十条
            $afterList = make(WorkMessageContract::class, [(int) $corpId])->getWorkMessagesRangeByCorpId($corpId, $message['from'], json_decode($message['tolist'], true)[0], $message['id'], '>', 10, 'id asc', ['id', 'from', 'msg_type', 'content', 'msg_time']);
        }
