...
class Destroy extends AbstractAction
{
     ...
     * })
     * @RequestMapping(path="/dashboard/roomCalendar/destroy", methods="DELETE")
     */
    public function handle(): array
    {
        ...
        Db::beginTransaction();
        try {
            $this->roomCalendarService->deleteRoomCalendar((int) $id);
            $this->roomCalendarPushService->deleteRoomCalendarPushByByRoomCalendarId((int) $id);
            Db::commit();
        } catch (\Throwable $e) {
            Db::rollBack();
            $loggers = $this->container->get(StdoutLoggerInterface::class);
            $loggers->error(sprintf('%s [%s] %s', '群日历删除失败', date('Y-m-d H:i:s'), $e->getMessage()));
            $loggers->error($e->getTraceAsString());
            throw new CommonException(ErrorCode::SERVER_ERROR, '群日历删除失败'); //$e->getMessage()
        }
