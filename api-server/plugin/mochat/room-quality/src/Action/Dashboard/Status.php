...
class Status extends AbstractAction
{
     ...
     * })
     * @RequestMapping(path="/dashboard/roomQuality/status", methods="put")
    ...
    public function handle(): array
    {
        ...
        try {
            ## 创建活动
            $this->roomQualityService->updateRoomQualityById((int) $params['id'], ['status' => (int) $params['status']]);

            Db::commit();
        } catch (\Throwable $e) {
            Db::rollBack();
            $this->logger->error(sprintf('%s [%s] %s', '群聊质检状态修改失败', date('Y-m-d H:i:s'), $e->getMessage()));
