...
class Destroy extends AbstractAction
{
     ...
     * })
     * @RequestMapping(path="/dashboard/roomQuality/destroy", methods="DELETE")
     */
    public function handle(): array
    {
        ...
        Db::beginTransaction();
        try {
            $this->roomQualityService->deleteRoomQuality($id);
            Db::commit();
        } catch (\Throwable $e) {
            Db::rollBack();
            $loggers = $this->container->get(StdoutLoggerInterface::class);
            $loggers->error(sprintf('%s [%s] %s', '群聊质检删除失败', date('Y-m-d H:i:s'), $e->getMessage()));
            $loggers->error($e->getTraceAsString());
            throw new CommonException(ErrorCode::SERVER_ERROR, '群聊质检删除失败' . $e->getMessage()); //$e->getMessage()
        }
