 ...
 */
namespace MoChat\Plugin\ChannelCode\QueueService;
...
class QrCodeUpdateApply
{
    ...
    private function handleQrCode(array $wxQrCode, int $channelCodeId)
    {
        try {
            ## 更新到数据表
            make(ChannelCodeContract::class)->updateChannelCodeById($channelCodeId, ['qrcode_url' => $wxQrCode['qrCodePath'], 'wx_config_id' => $wxQrCode['wxConfigId']]);
        } catch (\Exception $e) {
